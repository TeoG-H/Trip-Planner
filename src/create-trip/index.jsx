...
function CreateTrip() {
  ...
  setFormData((p) => {
    ...
    //daca in lista este inclusa valoarea cu care apelez functia atunci o elimin daca nu o adaug
    const updatedValues = currentValues.includes(value) ? currentValues.filter((item) => item !== value) // scoate
      : [...currentValues, value]; // adauga

    return { ...p, [name]: updatedValues,};
  });
};
  ...
  const OnGenerateTrpi = async () => {
      ...
      .replace("{transportMode}", formData.transportMode)
      .replace("{travelPace}", formData.travelPace)
      .replace("{travelerType}", formData.travelType)
  ...
  const SaveAiTrip=async(TripData)=>{
    ...
    // in ce baza de date o salvez, in ce colectie si numele documentului din colectie si datele din document
  ...
  return (
    <div className="relative min-h-screen overflow-hidden">
      ...
      {/* pointer-events-non nu intercepteaza clickuri, e pus la left, top 0 si -z-10 e in spatele continutului */}
      ...
      <div ref={heroRef} className="transition-all duration-700 ease-out mt-16 px-8 max-w-5xl mx-auto  mb-10 ">
        <h2 className="font-extrabold text-t tracking-tight">
        Let’s plan your next adventure
        </h2>

        <p className="mt-5 text-h3 text-gray-500 max-w-3xl">
        ...
        <div className="mt-16 flex flex-col gap-14 md:gap-20">
          ...
          <div>
            <h2 className="text-h2 my-3 font-bold">
          ...
          <div  ref={budgetRef}>
            <h2 className="text-h2 my-5 font-bold">
              What is your budget?
            </h2>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-5 mt-5">
              ...
              {SelectBudgetOptions.map((item, index) => (
                <div key={index} onClick={ () => handleInputChange("budget", item.title /*item.title din constante*/ ) }
                  className={`p-6 rounded-2xl border bg-white/80 backdrop-blur-sm cursor-pointer transition-all duration-500 hover:-translate-y-1 hover:shadow-xl
                ...
                >
                  ...
                  <h2 className="font-semibold text-cardT">{item.title}</h2>
                  <h2 className="text-cardD text-gray-500 mt-1">{item.desc}</h2>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-5 mt-5">
              {SelectTravelesList.map((item, index) => (
                ...
                >
                  ...
                  <h2 className="font-bold text-cardT">{item.title}</h2>
                  <h2 className="text-cardD text-gray-500">{item.desc}</h2>
                </div>
              ))}
            </div>
          </div>
          ...
          <div ref={paceRef}>
            <h2 className="text-h2 my-5 font-bold"> How would you like to pace your trip? </h2>
          ...
          <div ref={typeRef}>
            <h2 className="text-h2 my-5 font-bold"> Who are you traveling with? </h2>
          ...
          <div ref={preferenceRef}>
            <h2 className="text-h2 my-5 font-bold"> What type of experiences do you enjoy the most? </h2>
          ...
          <div ref={buttonRef} className="mt-10 justify-end flex">
            ...
            className=" px-10 py-6 rounded-full text-lg font-semibold bg-gradient-to-r from-[#2aa7c9] to-[#5fd3e6]
            hover:shadow-[0_20px_40px_rgba(95,211,230,0.35)] transition-all duration-300 ">
